...
//
//  - Production (Vercel frontend + Railway backend): NEXT_PUBLIC_BACKEND_URL is set
//    at build time to the Railway backend's public URL, so the browser calls the
//    backend directly.
//
//  The backend must allow the Vercel frontend origin in its CORS_ORIGINS env var.
...
export async function deleteCard(
  ...
  return fetchJSON(
    `${BASE}/decks/${deckId}/cards/${cardId}`,
    {
      method: 'DELETE',
    }
  );
...
export async function getReviewSession(
  ...
  return fetchJSON(
    `${BASE}/decks/${deckId}/review/session`
  );
}
...
export async function submitReview(
  ...
  return fetchJSON(
    `${BASE}/decks/${deckId}/review/${cardId}`,
    {
      method: 'POST',
      body: formData,
    }
